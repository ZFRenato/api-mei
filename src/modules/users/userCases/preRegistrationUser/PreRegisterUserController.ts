...
class PreRegisterUserController {
    ...
    async handle (request: Request, response: Response): Promise<Response> {
        ...
        try {
            const result = await this.preRegisterUserCase.execute({ name, phone, email, type, password });
            ...
            logger.error(error);
            return response.status(500).json({ error: error.message == 'Email Already registration' ? error.message : 'registration process failed' })
        }
